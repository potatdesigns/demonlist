...
/* =====================================================================
   ...
   points, etc.) that js/api-aredl.js reads instead of hitting AREDL's
   live API on every single visitor's page load. Every visitor was
   independently re-fetching the exact same ~1600-entry list; this makes
   that one shared, scheduled fetch instead (see
   .github/workflows/refresh-aredl-cache.yml, hourly) — and only keeps
   the slice this app actually tracks/displays, so the cache file (and
   everything downstream: the YouTube showcase/verifier cache, git diffs
   on every refresh) stays sized to LEVEL_LIST_SIZE, not AREDL's full
   list. See the comment on CONFIG.LIST_SIZE in js/config.js for why 150.
   ...
   staggering logic needed: it's just "fetch the whole list, keep the top
   LEVEL_LIST_SIZE, write it out" every run. Hourly is already far more
   often than levels actually get reordered/added, but since it costs
   nothing there's no reason to be stingier than that.
   ...
   Caching that too would mean pre-fetching detail for all LEVEL_LIST_SIZE
   levels on every run, which is a lot more work for a lot less benefit
   than just caching the list.
...
async function main() {
  ...
  const levels = fullList.slice(0, LEVEL_LIST_SIZE);

  const cache = { generatedAt: new Date().toISOString(), levels };
  await mkdir(path.dirname(CACHE_PATH), { recursive: true });
  await writeFile(CACHE_PATH, JSON.stringify(cache, null, 2) + '\n');

  console.log(`Wrote top ${levels.length} of ${fullList.length} AREDL levels to ${CACHE_PATH}.`);
}
